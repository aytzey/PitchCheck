...
Before the LLM sees anything, a deterministic **neural × research synthesis** layer links this pitch's TRIBE geometry to published findings: weak self/value response → the Falk et al. self-relevance lever; low fluency → Alter & Oppenheimer; reward-vs-social route dominance → Cohen et al. 2024; and the temporal-trace shape is classified into archetypes (strong-open-fade, late peak, buried lede, flat) per Chan et al. 2024. The synthesis is citation-anchored, appears in the report's deep dive, drives the refine brief, and the evaluator is instructed to execute its strongest levers in the top moves. The LLM produces the verdict, narrative, strength/risk analysis, a structured context-fit read (persona pain alignment, objection coverage, proof credibility, CTA ease, channel fit), and rewrite suggestions.
